$(function() {
	...
	$(".new_issue input[name='commit'], .new_issue input[name='continue']").click(function(event) {
		...
		if ( $("#issue_tracker_id").val() == 5 && $("#issue_is_private").prop('checked') == false ) {
			...
			$("#rah-dialog").dialog({
				...
				buttons: {
					"Create ticket": function() {
						$('<input>').attr({type: 'hidden', name: current_name_button, value: '1'}).appendTo( the_issue_form );
			...
			// Set right text for dialog
			$('#dynamic-text').html( redmine_ah.no_assignement );
	...
	$(".edit_issue input[name='commit']").click(function(event) {
		...
		if ( $("#issue_tracker_id").val() == 5 && $("#issue_is_private").prop('checked') == false ) {
			...
			// Set right text for dialog
			$('#dynamic-text').html( redmine_ah.same_assignement );
